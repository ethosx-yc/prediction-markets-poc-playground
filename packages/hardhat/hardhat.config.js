...
const { isAddress, getAddress, formatUnits, parseUnits } = ethers;
...
module.exports = {
	...
	networks: {
		localhost: {
			...
			accounts: {
				mnemonic:
					"black still carbon dentist gravity license captain math resemble runway team session",
					// mnemonic(),
		...
		mainnet: {
			url: "https://mainnet.infura.io/v3/6904b00d829c4a3a969fa0241ce70f62", // <---- YOUR INFURA ID! (or it won't work)
...
task(
	...
	.setAction(async (taskArgs, { network, ethers }) => {
		...
		while (contract_address.indexOf(taskArgs.searchFor) != 0) {
			...
			address =
				"0x" +
				EthUtil.privateToAddress(wallet._privKey).toString("hex");
...
task(
	...
	async (_, { ethers }) => {
		...
		try {
			...
			console.log(
				`--- Looks like there is no mnemonic file created yet.`
			);
			console.log(
